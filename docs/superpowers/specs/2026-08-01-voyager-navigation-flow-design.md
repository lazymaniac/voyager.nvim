...
Each logical action receives a monotonically increasing request token and stores its origin node ID and normalized result identities. A single definition/declaration/implementation/type-definition result is selected immediately after Voyager reproduces the native jump, including Neovim's jumplist and tagstack update. A standard quickfix/location-list item keeps its original `Location` or `LocationLink`, while a call item keeps its original call-hierarchy response; both add a nested Voyager request token and location-node ID in `user_data`. A session observer accepts a selection only when the active list ID/index points at that tagged item and the entered cursor matches it. This prevents unrelated movement to the same coordinate from changing the flow.
