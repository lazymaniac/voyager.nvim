...
Selecting an active tagged list item changes current only after an attributable user activation: `<CR>` or double-click in the observed list, or a non-compound user-entered quickfix/location-list jump command recognized at `CmdlineLeave`. The scheduled verifier then requires the list ID, active tagged item, and entered cursor to match. Unrelated movement to the same coordinate does nothing.

Assert the quickfix/loclist title and `context = { bufnr = context.bufnr, method = action.method }` match `vim.lsp.LocationOpts.OnList`. After opening, capture the concrete list ID and install temporary input/command-line activation listeners. Prove that list `<CR>`, location-list `<CR>`, double-click, typed `:cc` at the unchanged initial index, and a typed non-initial jump command arm selection; an unrelated `:edit`/`:buffer`, compound command, programmatic buffer switch, and post-hoc coordinate match do not. Change each of ID, nested tag, generation, request token, node lookup, and entered cursor independently and assert no `set_current`; only the exact active tuple with valid activation provenance may select once. Document that Neovim exposes no passive event for programmatic `vim.cmd()` jumps or arbitrary `<Cmd>`/Lua callback mappings, whose integrations must use `navigation.on_list` and its supplied `select(item)` callback.
...
For a list, call `setqflist` plus `botright copen`, or `setloclist(owner_winid, ...)` plus `lopen`, then record `{ kind, owner_winid, list_id, generation, request_token, presentation_token }`. A temporary `vim.on_key` listener arms list `<CR>`/double-click against that ID and item index. A temporary `CmdlineLeave` listener rejects aborted or compound input, canonicalizes the command with `nvim_parse_cmd`, and arms only the documented quickfix/location-list jump commands for the matching list kind. The scheduled verifier resolves that exact active list, validates the active item's nested tag and `resolve_node(node_id)`, and compares the entered buffer/one-based row/zero-based byte column with the canonical node before `set_current`. Every success, replacement, or invalidation removes both listeners. Do not select merely because an unrelated cursor happens to share coordinates.
